...
export const getProblemSetSearchByTitle = async (keyword: string, page: number = 0, size: number = 10) => {
    if (!keyword) return { page: { number: 0, size, totalElements: 0, totalPages: 0 }, problemSetList: [] };

    const url = `/api/v1/problem-sets/search/by-title?keyword=${encodeURIComponent(keyword)}&page=${page}&size=${size}`;
